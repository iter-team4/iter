...
vi.mock("../../services/routeApi", () => ({
  getMyRoutes: (...args: any[]) => mockGetMyRoutes(...args),

  searchRoutes: (...args: any[]) => mockSearchRoutes(...args),

  saveRoute: (...args: any[]) => mockSaveRoute(...args),

  deleteRoute: (...args: any[]) => mockDeleteRoute(...args),
}));
...
describe("useRoutes", () => {
  ...
  it("loads routes on mount", async () => {
    ...
    await waitFor(() => {
      expect(result.current.routesLoading).toBe(false);
    });

    expect(result.current.savedRoutes).toHaveLength(1);

    expect(result.current.savedRoutes[0].routeName).toBe("Morning Run");

    expect(mockGetMyRoutes).toHaveBeenCalledTimes(1);
  });

  it("handles failed route loading", async () => {
    mockGetMyRoutes.mockRejectedValue(new Error("API failed"));
  ...
  it("searches routes", async () => {
    ...
    expect(mockSearchRoutes).toHaveBeenCalledWith("park");

    expect(result.current.searchResults).toHaveLength(1);

    expect(result.current.searchResults[0].routeName).toBe("Park Route");
  });

  it("handles failed route searching", async () => {
    mockSearchRoutes.mockRejectedValue(new Error("Search failed"));
  ...
  it("sets selected saved route", () => {
    ...
    expect(result.current.selectedSavedRoute).toEqual(route);
  });

  it("exposes saveRoute function", () => {
    const { result } = renderHook(() => useRoutes());

    expect(result.current.saveRoute).toBeDefined();

    expect(typeof result.current.saveRoute).toBe("function");
  });

  it("exposes deleteRoute function", () => {
    const { result } = renderHook(() => useRoutes());

    expect(result.current.deleteRoute).toBeDefined();
